...
def gen_constraint_4(x, m_bs, n_ue, beta, simTime, gen_dict, interval=1):
    bs_pairs = []
    for i in range(m_bs):
        for j in range(m_bs):
            if i != j:
...
def add_all_constraints(model, Vars, nodes, network, SNR, beta, R, scenario):
    m_bs = len(network) 
    x = Vars[0]
    y = Vars[1]
    constrs_dict = {}
    ...
    n_ue = len(nodes)
    gen_constraint_4(x, m_bs, n_ue, beta, scenario['simTime'], constrs_dict)
